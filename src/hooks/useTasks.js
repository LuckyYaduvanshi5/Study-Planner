...
export const useTasks = (user) => {
  ...
  // Add a new task
  const addTask = async ({ name, description, dueDate, priority }) => {
    if (!user) return { error: 'User not authenticated' };

    try {
      setError(null);
      
      // Changed 'name' to 'task_name' to match database schema
      const newTask = {
        user_id: user.id,
        task_name: name,         // Changed from 'name' to 'task_name'
        description: description,
        due_date: dueDate,       // Changed from 'dueDate' to 'due_date'
        priority: priority,
        completed: false,
